...
// Constantes
#define MMAX	4				//Max cantidad de coeficientes de los controladores
#define NMAX	4
...
int _ek_rpm[M+1];
int _uk_vel_v[N+1];   // uk[0] tension de exitacion del motor (OUTPUT CONTROLER)

int _ek_gra[MM+1];
int _uk_pos_v[NN+1];  // uk[0] tension de exitacion del motor (OUTPUT CONTROLER)
...
{
   int Yout;
   //Xin=FP(Xin,Q); // Pasamos Xin a Fixed Point
...
void loop_irq_FP(void)
{
   ...
   int yma_k_v, ym_k_v, duty_porcentual;
...
////Trabajamos con un controlador o con el otro dependiendo del tipo de control deseado.
   ...
   if(_ControlType==0)   //Velocidad = 0   /   Posicion = 1
   {
      ...
      //yma_k_v=lineal_FP((int)valorLeido_c, AD_Y_MAX_v, AD_Y_MIN_v, AD_X_MAX_c, AD_X_MIN_c);

      //// Convertimos de V analog adaptada (0V a 5V) a V medida (-10V a 10V)
      //ym_k_v=lineal_FP(yma_k_v, ADAPT_Y_MAX_v, ADAPT_Y_MIN_v, ADAPT_X_MAX_v, ADAPT_X_MIN_v);

      //// Convertimos de V medida (-10V a 10V) a Velocidad de salida [rpm]
      //_yk_rpm_o=lineal_FP(ym_k_v, TACOM_Y_MAX_rpm, TACOM_Y_MIN_rpm, TACOM_X_MAX_v, TACOM_X_MIN_v);
      //
	// Finalmente convertimos de (0_1023)c a (-100_+100)rpm en Fixed Point
      ...
      cola_FP(_ek_rpm, M+1,ek_actual_rpm);
      cola_FP(_uk_vel_v, N+1, NO_VALIDO);   // NO_VALIDO será sobreescrito por GdeZ (uk_actual_v)   
   ...
   else
   {
      ...
      //// Convertimos de V medida (-10V a 10V) a Angulo de salida [grados]
      //_yk_gra_o=lineal_FP(ym_k_v, TITA_Y_MAX_gra, TITA_Y_MIN_gra, TITA_X_MAX_v, TITA_X_MIN_v);
     ...
     ek_actual_gra=consigna_gra_i-_yk_gra_o;  //Senal de error adecuada
      ...
      /////// 4 - Desplaza tablas temporales 
      cola_FP(_ek_gra, MM+1,ek_actual_gra);
      cola_FP(_uk_pos_v, NN+1, NO_VALIDO);   // NO_VALIDO será sobreescrito por GdeZ (uk_actual_v)   
