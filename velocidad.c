...
main()
{
	...
	while(1)
	{
		...
		cola(p_ek_rpm, M+1,ek_actual_rpm);
		cola(p_uk_v, N+1, NO_VALIDO);	// NO_VALIDO será sobreescrito por GdeZ (uk_actual_v)	


		/////// 5 - Regulador (GdeZ)
		// Prototipo: float GdeZ(float *b, float *a, float *u, float *y, int m, int n)
		//											 p_ek_rpm  p_uk_v
		uk_v[0]=GdeZ(p_b, p_a, p_ek_rpm, p_uk_v, M, N);
		

		/////// 6 - Convertimos de Vmotor deseada [-10V a +10V] a Duty Cycle[%]
		duty_porcentual=lineal(*p_uk_v, DUTY_MAX, DUTY_MIN, TACOM_X_MAX_v, TACOM_X_MIN_v);
